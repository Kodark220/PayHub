...
function buildReceiptLines(t: Tx) {
  return [
    "PayHub Receipt",
...
export function ConfPaymentsApp() {
  ...
  const [activeReceiptTx, setActiveReceiptTx] = useState<Tx | null>(null);
  ...
  async function downloadReceiptImage(t: Tx) {
    ...
    ctx.fillText("PayHub Receipt", 80, 130);
    ...
    const lines = buildReceiptLines(t);
    lines.forEach((line, index) => {
      ctx.fillText(line, 100, 280 + index * 90);
    });

    ctx.fillStyle = "#A0A0A0";
    ctx.font = "400 28px Arial";
    ctx.fillText("Generated by PayHub", 100, 1450);

    const link = document.createElement("a");
    link.href = canvas.toDataURL("image/png");
    link.download = `payhub-receipt-${t.id}.png`;
    link.click();
    toast.success("Receipt image downloaded");
  }

  function downloadReceiptPdf(t: Tx) {
    const lines = buildReceiptLines(t);
    const html = `
      <html>
        <head>
          <title>PayHub Receipt</title>
          <style>
            body { font-family: Arial, sans-serif; padding: 32px; color: #111; }
            h1 { margin: 0 0 18px; color: #FF6B00; }
            p { margin: 8px 0; font-size: 14px; }
          </style>
        </head>
        <body>
          <h1>PayHub Receipt</h1>
          ${lines.map((line) => `<p>${line.replace(/</g, "&lt;").replace(/>/g, "&gt;")}</p>`).join("")}
          <p style="margin-top:24px;color:#666">Generated by PayHub</p>
        </body>
      </html>
    `;
    const win = window.open("", "_blank");
    if (!win) {
      toast.error("Popup blocked. Enable popups to export PDF.");
      return;
    }
    win.document.open();
    ...
    <main className="mx-auto min-h-screen w-full max-w-[390px] bg-[#0A0A0A] px-4 pb-28 pt-6 text-white">
      ...
      {view === "home" && (
        <section className="space-y-4">
          ...
          <div><div className="mb-2 flex items-center justify-between"><p className="text-sm">Recent</p><button className="text-xs text-[#FF6B00]" onClick={() => { setView("history"); setTab("history"); }}>See all</button></div><div className="space-y-2">{txs.slice(0, 3).map((t) => <TxCard key={t.id} t={t} onReceipt={openReceipt} />)}</div></div>
        </section>
      )}
      ...
      {view === "history" && <HistoryScreen filter={filter} setFilter={setFilter} search={search} setSearch={setSearch} history={history} walletUsdcBalance={walletUsdcBalance} walletEthBalance={walletEthBalance} onReceipt={openReceipt} />}
      ...
      <AnimatePresence>
        {activeReceiptTx && (
          <ReceiptSheet
            ...
            onImage={downloadReceiptImage}
            onPdf={downloadReceiptPdf}
          />
        )}
      </AnimatePresence>
    </main>
...
function displayTxTitle(title: string) {
...
function TxCard({t, onReceipt}: {t: Tx; onReceipt?: (tx: Tx) => void}) {
...
function ReceiptSheet({tx, onClose, onImage, onPdf}: {tx: Tx; onClose: () => void; onImage: (tx: Tx) => void; onPdf: (tx: Tx) => void}) {
  return (
    <motion.div initial={{opacity: 0}} animate={{opacity: 1}} exit={{opacity: 0}} className="fixed inset-0 z-50 bg-black/60 p-4">
      <motion.div initial={{y: 50}} animate={{y: 0}} exit={{y: 50}} className="mx-auto mt-24 w-full max-w-[390px] rounded-2xl border border-[#1F1F1F] bg-[#111111] p-4">
        <p className="text-center text-sm font-medium">Download Receipt</p>
        <div className="mt-3 rounded-xl border border-[#1F1F1F] bg-[#0A0A0A] p-3 text-xs text-[#A0A0A0]">
          <p className="text-white">{displayTxTitle(tx.title)}</p>
          <p>{tx.time}</p>
          <p>{tx.local} • {tx.usdc}</p>
        </div>
        <div className="mt-4 grid grid-cols-2 gap-2">
          <button className="rounded-xl border border-white/30 py-2 text-sm" onClick={() => onImage(tx)}>Image (PNG)</button>
          <button className="rounded-xl bg-[#FF6B00] py-2 text-sm font-semibold" onClick={() => onPdf(tx)}>PDF</button>
        </div>
        <button className="mt-2 w-full rounded-xl border border-[#1F1F1F] py-2 text-sm text-[#A0A0A0]" onClick={onClose}>Close</button>
      </motion.div>
    </motion.div>
  );
}
